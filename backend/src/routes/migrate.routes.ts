...
async function extractTourData(api: AviannaApiClient, tourId: number, sessionId?: string): Promise<any> {
    if (sessionId) sendSSE(sessionId, 'step', { step: 1, status: 'active', message: 'Загрузка данных тура...' });

    console.log(`\n  📥 Загрузка тура ID: ${tourId}`);
    const tour = await api.getTour(tourId);

    if (!tour) {
        if (sessionId) sendSSE(sessionId, 'step', { step: 1, status: 'error', message: 'Не удалось загрузить тур' });
        console.log('  ❌ Не удалось загрузить тур');
        return null;
    }

    console.log(`  ✅ Тур: ${tour.name}`);
    if (sessionId) sendSSE(sessionId, 'step', { step: 1, status: 'active', message: `Тур найден: ${tour.name}. Загрузка данных...` });
    ...
    if (tour.transport_forth_id) {
        if (sessionId) sendSSE(sessionId, 'step', { step: 1, status: 'active', message: 'Загрузка транспорта...' });
    ...
    if (tour.hotels && tour.hotels.length > 0) {
        if (sessionId) sendSSE(sessionId, 'step', { step: 1, status: 'active', message: `Загрузка гостиниц (${tour.hotels.length})...` });
    ...
    if (sessionId) sendSSE(sessionId, 'step', { step: 1, status: 'active', message: 'Загрузка названий городов...' });
    ...
    if (serviceIds.size > 0) {
        if (sessionId) sendSSE(sessionId, 'step', { step: 1, status: 'active', message: `Загрузка услуг (${serviceIds.size})...` });
    ...
    if (sessionId) sendSSE(sessionId, 'step', { step: 1, status: 'completed', message: `Тур "${tour.name}" загружен полностью` });
...
async function performMapping(
    ...
    if (sessionId) sendSSE(sessionId, 'step', { step: 3, status: 'active', message: 'Поиск соответствий в Pazl Tours...' });
    ...
    if (cityIds.size > 0) {
        if (sessionId) sendSSE(sessionId, 'step', { step: 3, status: 'active', message: `Маппинг городов (${cityIds.size})...` });
    ...
    // 5. Гостиницы
    if (tourData.hotels) {
        if (sessionId) sendSSE(sessionId, 'step', { step: 3, status: 'active', message: `Маппинг гостиниц (${tourData.hotels.length})...` });
        console.log('\n  🏨 Гостиницы:');
    ...
    if (tourServicesToFind.size > 0) {
        if (sessionId) sendSSE(sessionId, 'step', { step: 3, status: 'active', message: `Маппинг услуг (${tourServicesToFind.size})...` });
        console.log('\n  🛠️ Услуги тура:');
    ...
    if (sessionId) sendSSE(sessionId, 'step', { step: 3, status: 'completed', message: `Маппинг завершён. Найдено: ${totalExists}, создать: ${totalToCreate}` });
...
async function createMissingEntities(
    ...
    if (sessionId) sendSSE(sessionId, 'step', { step: 4, status: 'active', message: 'Создание недостающих сущностей...' });
    ...
    if (citiesToCreate.length > 0) {
        ...
        for (const city of citiesToCreate) {
            if (sessionId) sendSSE(sessionId, 'step', { step: 4, status: 'active', message: `Создание города: ${city.source_name}` });
    ...
    if (categoriesToCreate.length > 0) {
        ...
        for (const category of categoriesToCreate) {
            if (sessionId) sendSSE(sessionId, 'step', { step: 4, status: 'active', message: `Создание категории: ${category.source_name}` });
    ...
    if (servicesToCreate.length > 0) {
        ...
        for (const service of servicesToCreate) {
            ...
            if (sessionId) sendSSE(sessionId, 'step', { step: 4, status: 'active', message: `Создание услуги: ${service.source_name}` });
    ...
    if (transportsToCreate.length > 0) {
        ...
        for (const transport of transportsToCreate) {
            ...
            if (sessionId) sendSSE(sessionId, 'step', { step: 4, status: 'active', message: `Создание транспорта: ${transport.source_name}` });
    ...
    if (hotelsToCreate.length > 0) {
        ...
        for (const hotel of hotelsToCreate) {
            ...
            if (hotelData) {
                if (sessionId) sendSSE(sessionId, 'step', { step: 4, status: 'active', message: `Создание гостиницы: ${hotel.source_name}` });
    ...
    if (sessionId) sendSSE(sessionId, 'step', { step: 4, status: 'completed', message: `Создано сущностей: ${createdEntities.size}` });
...
// ============================================
// MAIN API РОУТ (ЗАПУСКАЕТ МИГРАЦИЮ И ВОЗВРАЩАЕТ sessionId)
// ============================================
...
async function runMigration(
    ...
    try {
        ...
        console.log('✅ Вход в Pazl выполнен');
...
// Старый роут для обратной совместимости (прямой POST)
router.post('/', async (req: Request, res: Response) => {
    ...
    console.log('\n' + '='.repeat(80));
    console.log('🔄 МИГРАЦИЯ ТУРА AVIANNA → PAZL TOURS (прямой вызов)');
    console.log('='.repeat(80));
    ...
    try {
        ...
        if (aviannaPage.url().includes('/auth')) {
            await browser.close();
            return res.status(401).json({ success: false, error: 'Ошибка входа в Avianna' });
        }
        ...
        if (!toursResult?.data || toursResult.data.length === 0) {
            await browser.close();
            return res.status(404).json({ success: false, error: `Тур "${tourName}" не найден` });
        }
